...
params:
  sample: NULL  # Default value can be NULL or any default you deem fit
---
...
# batch_size = 100
# main_variants = synonymous_nonsynonymous_variants

load_vep_and_count_single <- function(file, col_names, variant_type, main_variants){
  
  # Function to sum and replace rows based on matched variant types
  ...
  # Load the VEP file
  vep <- fread(file, fill = TRUE, skip = 120, col.names = col_names, na.strings = "-")
  
  # Process the data to count variants
  vep.count <- vep %>%
  ...
  # Optional: Make consequence names easier to read if required
  if(variant_type == "Consequence"){
    vep.count$variant_type <- gsub(",", " / ", gsub("_", " ", vep.count$variant_type))
  }
...
load_vep_and_count_types <- function(files, sample_names, col_names, variant_type, main_variants, batch_size = 100){
  ...
  results_list <- foreach(file_chunk = file_chunks, .packages = c("data.table", "dplyr")) %dopar% {
    chunk_results <- lapply(file_chunk, function(file) {
      tryCatch({
        vep <- fread(file, fill = TRUE, skip = 105, col.names = col_names, na.strings = "-")
        vep.count <- vep %>% 
          select(Uploaded_variation, !!sym(variant_type)) %>%
          filter(!is.na(!!sym(variant_type)) & !!sym(variant_type) != "") %>%
          distinct(Uploaded_variation, .keep_all = TRUE) %>%
...
load_vep_and_count_types_synonymous_nonsynonymous_single <- function(file, col_names, variant_type, main_variants) {
  sum_and_replace_rows <- function(data, strings_to_match) {
    # Work on a copy of the data to avoid modifying the original data frame
    result_data <- data
    
    for (string_to_match in strings_to_match) {
      # Create a regular expression that matches the string at the start or before a comma
      pattern <- paste0("^", string_to_match, "(,|$)")
  
      # Identify rows where the variant type matches the pattern
      to_remove <- grepl(pattern, result_data$variant_type)
      
      # Sum the 'count' values for these rows
      sum_count <- sum(result_data$count[to_remove])
      
      # Remove the matched rows from the result data
      result_data <- result_data[!to_remove, ]
      
      # Append a new row with the summed value
      new_row <- data.frame(variant_type = string_to_match, count = sum_count)
      result_data <- rbind(result_data, new_row)
    }
    
    # Reorder the data frame after the operation
    result_data <- result_data[order(result_data$variant_type), ]
    
    return(result_data)
  }
  
  # Read the VEP output file
  vep <- fread(file, fill = TRUE, skip = 120, col.names = col_names, na.strings = "-")
...
# function to load vep files and count occurences of a specific variant type
# IMPROVEMENT: handle duplicated variants
load_vep_and_count_types_synonymous_nonsynonymous <- function(files, sample_names, col_names, variant_type, main_variants, batch_size = 100){
...
vep_observed_file <- paste0("/gs/gsfs0/users/rcutler/vijg-lab/2023-Ronnie/231009_multiple_ENU_analysis/variant-effect-prediction/observed/", sample, ".txt")

vep_simulated_files <- list.files(path = "/gs/gsfs0/users/rcutler/Software/variant-effect-predictor/SigProfilerSimulator_output_burden_matched_vep_array",
...
#### Count types
```{r,eval = FALSE}
observed_consequence <- load_vep_and_count_single(file = vep_observed_file,
...
```{r,eval = FALSE}
dir.create("/gs/gsfs0/users/rcutler/vijg-lab/2023-Ronnie/231009_multiple_ENU_analysis/variant-effect-prediction/output")
setwd("/gs/gsfs0/users/rcutler/vijg-lab/2023-Ronnie/231009_multiple_ENU_analysis/variant-effect-prediction/output")

consequence_oe_mean <- calc_OE_ratio_single(observed_consequence, 
                                             simulated_consequence, 
                                             get_summary_type = "mean")

write.csv(consequence_oe_mean, paste0(sample, "_consequence_oe_mean.csv"), row.names = FALSE)

consequence_oe_median <- calc_OE_ratio_single(observed_consequence, 
                                             simulated_consequence, 
                                             get_summary_type = "median")

write.csv(consequence_oe_median, paste0(sample, "_consequence_oe_median.csv"), row.names = FALSE)
```
...
write.csv(consequence_oe_mean, paste0(sample, "_consequence_main_oe_mean.csv"), row.names = FALSE)

consequence_oe_median <- calc_OE_ratio_single(observed_consequence, 
                                             simulated_consequence, 
                                             get_summary_type = "median")

write.csv(consequence_oe_median, paste0(sample, "_consequence_main_oe_median.csv"), row.names = FALSE)
```
...
#### Count types
```{r,eval = FALSE}
synonymous_nonsynonymous_variants <- c("synonymous_variant",
                                       "stop_gained",
                                       "stop_lost",
                                       "start_lost",
                                       "missense_variant", 
...
observed_consequence <- load_vep_and_count_types_synonymous_nonsynonymous_single(file = vep_observed_file,
                                    col_names = vep_cols_observed,
                                    variant_type = "Consequence",
                                    main_variants = synonymous_nonsynonymous_variants)
...
write.csv(consequence_oe_mean, paste0(sample, "_consequence_synonymous_nonsynonymous_oe_mean.csv"), row.names = FALSE)

consequence_oe_median <- calc_OE_ratio_single(observed_consequence, 
                                             simulated_consequence, 
                                             get_summary_type = "median")

write.csv(consequence_oe_median, paste0(sample, "_consequence_synonymous_nonsynonymous_oe_median.csv"), row.names = FALSE)
```
