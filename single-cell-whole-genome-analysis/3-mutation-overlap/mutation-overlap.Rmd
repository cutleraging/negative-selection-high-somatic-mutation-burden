---
title: "multiple ENU mutation overlap analysis"
...
## Shared SNVs between cells within the same cycle
```{r}
...
pdf("SNV_shared_observed.pdf",
    width = 4,
    height = 5)
ggplot(sample_table, aes(x = condition, y = SNV_obs_shared, fill = factor(cycle))) +
    geom_boxplot(outlier.shape = NA) +
    geom_point(position = position_jitterdodge(jitter.width = 0.2), size = 2, alpha = 0.5) +
    guides(fill = guide_legend(override.aes = list(shape = NA))) +
    labs(x = "Condition", y = "Shared SNVs Per Cell", fill = "Cycle") +
...
pdf("SNV_shared_estimated.pdf",
    width = 4,
    height = 5)
ggplot(sample_table, aes(x = condition, y = SNV_est_shared, fill = factor(cycle))) +
    geom_boxplot(outlier.shape = NA) +
    geom_point(position = position_jitterdodge(jitter.width = 0.2), size = 2, alpha = 0.5) +
    guides(fill = guide_legend(override.aes = list(shape = NA))) +
    labs(x = "Condition", y = "Estimated shared SNVs Per Cell", fill = "Cycle") +
    scale_fill_viridis(discrete = TRUE, end = 0.8) +
    theme_Publication(base_size = 16) + 
    theme(legend.justification = c("center","top"),
          legend.position = "top") +
...
Shared INDELs between cells within the same cycle
```{r}
# get list of variants for each cycle and put this into a list
...
pdf("INDEL_shared_observed.pdf",
    width = 4,
    height = 5)
ggplot(sample_table, aes(x = condition, y = INDEL_obs_shared, fill = factor(cycle))) +
    geom_boxplot(outlier.shape = NA) +
    geom_point(position = position_jitterdodge(jitter.width = 0.2), size = 2, alpha = 0.5) +
    guides(fill = guide_legend(override.aes = list(shape = NA))) +
    labs(x = "Condition", y = "Shared INDELs Per Cell", fill = "Cycle") +
    scale_fill_viridis(discrete = TRUE, end = 0.8) +
    theme_Publication(base_size = 16) + 
    theme(legend.justification = c("center","top"),
          legend.position = "top") +
...
pdf("INDEL_shared_estimated.pdf",
    width = 4,
    height = 5)
ggplot(sample_table, aes(x = condition, y = INDEL_est_shared, fill = factor(cycle))) +
    geom_boxplot(outlier.shape = NA) +
    geom_point(position = position_jitterdodge(jitter.width = 0.2), size = 2, alpha = 0.5) +
    guides(fill = guide_legend(override.aes = list(shape = NA))) +
    labs(x = "Condition", y = "Estimated shared INDELs Per Cell", fill = "Cycle") +
    scale_fill_viridis(discrete = TRUE, end = 0.8) +
    theme_Publication(base_size = 16) + 
    theme(legend.justification = c("center","top"),
          legend.position = "top") +
...
# convert to dataframe and output for later use
df_gr <- as.data.frame(gr_list_duplicates)
df_gr$REF <- sapply(df_gr$REF, function(x) paste(x, collapse=","))
df_gr$ALT <- sapply(df_gr$ALT, function(x) paste(x, collapse=","))
write.csv(df_gr, "shared_mutations.csv", row.names = FALSE)
```
...
df <- merge(df, sample_table, by.x = "Category", by.y = "sample")

pdf("SNV_shared_mutations_removed.pdf",
    width = 5,
    height = 5)
ggplot(df, aes(x = Category, y = Value, fill = condition)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  labs(x = "Sample", y = "Removed shared SNVs") +
  theme_Publication() +
  scale_fill_Publication() +
  scale_fill_viridis(discrete = TRUE, end = 0.8, option = "cividis")
dev.off()

pdf("SNV_shared_mutations_removed_percentage.pdf",
    width = 5,
    height = 5)
ggplot(df, aes(x = Category, y = Value/Total , fill = condition)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  labs(x = "Sample", y = "% Removed shared SNVs") +
  theme_Publication() +
...
## Modify the sample table with the new mutation counts and estimations
```{r}
hg19_size <- 3036303846
...
## Filtered VCF
```{r}
setwd("/Users/ronaldcutler/Dropbox\ (EinsteinMed)/Vijg-lab/Projects/mutation\ accumulation/231009\ multiple\ ENU\ analysis/1-data/final/filtered")

for(i in 1:length(mutations_filtered)){
  export_GRanges_to_VCF(mutations_filtered[[i]], names(mutations_filtered)[i])
}
```
